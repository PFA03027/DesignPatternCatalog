 ...
 * 一方で、プロセスの終了が保証されないようなシステムでは、SingltonクラスでOpen/Close戦略が取りづらくなってしまいます。
 * 特に、Close戦略をどう実装するか、また、いつ実施するか？　というのが課題になります。
 *
 * 単純にClose戦略を実行するI/Fをメンバ関数に導入してしまうと、誰でもClose戦略を実行できてしまいます。
 * そのため、すべての利用者は、利用前に、Open戦略実施済みかどうかのチェックが必要になり、かなり面倒です。
 * これに対しては、Close戦略を実行するI/Fはprotected or privateにアクセス制限を行ったうえで、実行タイミングを設計で保証するというアプローチが必要になります。
 *
 * ご参考
