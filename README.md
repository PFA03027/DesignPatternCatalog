...
また、実装の隠蔽方法として、pimplイディオムで実装の詳細を隠す方法を採用しました。

## Proxy
### Classification: Structure
* C++ での実装(virtual proxyとしての実装例): StructurePattern07_Proxy/cpp_src

あるオブジェクトへのアクセスを制御するために、その代理オブジェクトを提供するデザインパターンです。
アクセス制御する動機としていくつかあります。GoF本では、下記の例が上がっています。
* remote proxy: 処理を行う実態が別のプロセスにあり、別プロセスのI/Fの代理オブジェクトを提供する。
* vitrual proxy: オブジェクトの構築にコストがかかるが、すぐに必要とされない場合、遅延ロード等を行う代理オブジェクトを提供する。
* protection proxy: 実オブジェクト毎にアクセス権を制御する代理オブジェクトを提供する。
* smart reference: いわゆるスマートポインタ

代理オブジェクトは、Realなオブジェクトと同じI/Fを提供します。
そのため、代理オブジェクトにアクセスるクライアント側からは、そのオブジェクトがRealなオブジェクトなのか、代理オブジェクトなのかを区別せず扱えるようになることがメリットです。
代理オブジェクトを提供する側も、クライアント側から独立して内部のアクセス方法を追加、変更することも可能になります。

ただし、実装において気を付けなければならない点があります。
Proxyパターンが直接提供するのはI/Fの代理です。そのため、I/Fの呼び出しに伴う副作用に大きな違いがある場合は、その影響を許容できるかどうかを検討する必要があります。
例えば、RealなオブジェクトのI/Fの実行では定数時間となるところ、代理オブジェクトのI/Fの実行においては、ブロッキングAPIとしてふるまう等です。
（代理オブジェクトによって隠蔽された外部依存とのアクセスで、外部依存が特定の状態になるまでAPIがブロッキングされるような実装が該当する）
